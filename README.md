...
- `npm run check` checks Biome lint and formatting; `npm run fix` applies its fixes. `npm run build` emits ESM JavaScript and declarations to `dist/`, with `.js` relative imports for NodeNext. Tests run through tsx and are not typechecked by that build.
- `npm run bench` performs **single-shot** measurements of the release implementation, not `createSolver` or enumeration benchmarks. It writes only [`test/phase4-benchmark.json`](test/phase4-benchmark.json) and [the comparison report](test/phase4-benchmark.md), preserving frozen Phase-2/3 evidence. It authenticates historical commit/blob/SHA-256 identities, hashes actual current source/fixture bytes, and independently checks SAT models. HEAD is context, not a substitute for source fingerprints.
