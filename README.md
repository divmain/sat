...
Solve Boolean satisfiability problems with a zero-runtime-dependency, TypeScript/ESM library. It compiles expressions to CNF with a Tseitin transformation and uses an iterative **conflict-driven clause learning (CDCL)** solver: two-watched-literal propagation, first-UIP learning with recursive minimization, non-chronological backtracking, VSIDS, phase saving, deterministic Glucose-style EMA restarts with blocking, and two-tier learned-clause reduction. Find one model, enumerate all models, compile once and solve repeatedly under different assumptions, or conjoin further constraints into a live solver. Every entry point returns a rich result object, and work can be bounded with conflict budgets or interrupted asynchronously.
...
The solving functions take an expression, not a bare string: use `and('a')` to assert one variable. `implies` and `xor` are constructor sugar over `and`/`or`/`not`; the formula shapes are unchanged from v1. Treat an expression as input, not as a live way to modify an already-created solver: compilation **snapshots** the expression, so editing the object afterwards never changes an existing solver, and a later `SatSolver.add()` of the edited object observes its new contents.
...
Every entry point has an async twin — `getSolutionAsync`, `getAllSolutionsAsync`, and `SatSolver.solveAsync` — with identical semantics driven in `yieldQuantum` work slices (default 4096, clamped to a minimum of 64) and platform-neutral event-loop yields between them. Validation runs before the first yield, and every error — validation, heuristic callback, or scheduler — rejects the returned Promise rather than throwing synchronously. An optional `AbortSignal` aborts between slices, and `conflictBudget` bounds the conflicts one call may spend; both produce `{ status: 'unknown', reason: 'aborted' | 'conflictBudget' }` instead of a verdict. Async and sync calls agree exactly for identical non-aborted histories.

Interruptibility has explicit limits. Yields occur only at safe points between atomic units of work: between clause inspections, between individual assumption-replay steps, after complete conflict transactions, and between enumeration models. Compilation, constructor unit installation, and each individual atomic search operation (one clause inspection, one conflict analysis) always run to completion synchronously, so `yieldQuantum` bounds work between yields, never wall-clock latency. A pre-aborted signal still validates its options and consults already-established verdicts (a cached or compiled UNSAT still returns `'unsat'`), then resolves `{ status: 'unknown', reason: 'aborted' }` without propagation or search. An aborted or exhausted solver stays coherent and reusable — legitimately acquired learned clauses and heuristic state persist.
...
Compiles once and uses a fresh solver for this call. `SolveOptions` accepts `assumptions`, `variablePriority`, a writable `stats` out-parameter (sparse objects are zero-filled), and a `conflictBudget`: a non-negative safe-integer conflict limit for this call (`undefined` is unlimited; negative, fractional, `NaN`, or infinite values are descriptive validation errors, raised before any cached-UNSAT shortcut). A positive budget is a **hard conflict limit**: each in-budget conflict completes its atomic analysis/learning/backjump transaction, and once the budget is spent the solver starts no further propagation pass or search step merely to seek a verdict — exhaustion returns `{ status: 'unknown', reason: 'conflictBudget' }` after the final atomic conflict transaction. As an explicit startup exception, `conflictBudget: 0` still permits the initial root-propagation pass — one allowance per call, not renewed after learning or between enumeration models — so a compiled UNSAT, an initial root conflict, or an already-complete model can still return a verdict; otherwise the call returns `'unknown'` without making a search decision. Returns a `SolveResult`: one complete model on `'sat'`, or the failed-assumption core on `'unsat'`.

Startup unit propagation is followed by **pure-literal elimination (PLE)**: each round assigns all currently pure variables in an index-ordered batch, propagates, and repeats to a fixpoint. PLE is enabled **only in the single-shot `getSolution`/`getSolutionAsync` entry points** — never in enumeration or incremental solving, where persistent pins could silently drop valid models. It preserves satisfiability, not the set of models, so it can change which valid model is returned. Named don't-cares are completed by ordinary decisions.
...
`SatSolver.add(expr)` conjoins further constraints into the same handle without recompiling or losing learned state: see [Adding Constraints Incrementally](#adding-constraints-incrementally). New names are sorted within each added batch and appended after all previously assigned internal indices, so the internal index order is deterministic but **history-dependent**; `variables()` returns the globally sorted named set regardless. Each `add()` snapshots the expression as admitted: mutating a caller-owned expression object never retroactively changes admitted constraints, and an edited object passed to a later `add()` is observed in its new state. An `add` that throws (for example, a malformed hand-built expression) is failure-atomic: the handle is byte-for-byte unchanged and stays reusable. `add` requires a quiescent handle — it throws during an in-flight async solve or from a `variablePriority` hook — and it reports no stats: like construction, its work is excluded from every per-call `stats` measurement. It invalidates derived state from earlier calls: models, cores, and `variables()` snapshots describe the pre-add formula. This handle has no enumeration, reset, or disposal method; drop it when no longer needed.
...
| `learnedLiterals` | Post-minimization literals produced by conflict analysis in the measurement scope, counted before duplicate suppression — a rediscovered live clause still contributed its literals. |
| `minimizedLiterals` | Literals removed from learned clauses by recursive minimization in the measurement scope; `learnedLiterals + minimizedLiterals` is the pre-minimization total produced by analysis. |
...
Version 3.0.0 keeps the v2 formula API — the constructors, `BooleanExpr`, `Value`, complete numeric models, and assumption validation are unchanged — and makes one intentional breaking change to solving: **every entry point returns a rich result object**, and `SatSolver.solve` takes an options bag. No compatibility shims exist. The "before" snippets below are historical references, not runnable examples.

| v2 code | v3 migration |
| --- | --- |
| `const model = getSolution(expr, options)` | `const result = getSolution(expr, options)` — narrow on `result.status`: `'sat'` carries `result.model`; `'unsat'` carries `result.core`; `'unknown'` carries `result.reason`. The `model \| null` return is gone. |
| `if (model === null)` | `if (result.status === 'unsat')` — and the result now carries `core`: a subset of the call's assumptions (same values) that suffices for UNSAT. `{}` appears only when no assumptions were supplied or UNSAT was proven independent of them. Cores are sound, not necessarily minimal. |
| `const models = getAllSolutions(expr, options)` | `const result = getAllSolutions(expr, options)` — on `result.status === 'complete'`, `result.models` is exactly the v2 array (`[]` for UNSAT). `'unknown'` adds `reason` and returns the partial model prefix in `models` (a non-empty prefix still proves SAT; only completeness is undetermined). |
| `solver.solve(assumptions, stats)` | `solver.solve(assumptions, { stats })` — the second argument is now a `SatSolverCallOptions` bag (`{ stats?, conflictBudget? }`); the bare positional stats argument is gone. The return value changes from `model \| null` to `SolveResult`, as above. |
| `SolveOptions` | Same fields (`assumptions`, `variablePriority`, `stats`), plus optional `conflictBudget`: a hard, per-call, non-negative-conflict limit. Exhaustion returns `'unknown'` after the final atomic conflict transaction instead of v2's internal, throwing `maxConflicts` knob (which was never exported and is now retired). |
| `stats` out-parameters (`SolverStats`) | Now sparse (`SolverStatsInput`): missing fields are zero-filled, then **all** fields are populated — a partial literal is always left complete. Two new counters, `learnedLiterals` and `minimizedLiterals`, make learned-clause minimization observable. `createSolverStats()` returns a complete all-zero object. |
| — (no v2 equivalent) | Async twins `getSolutionAsync`, `getAllSolutionsAsync`, and `SatSolver.solveAsync` add `signal`/`yieldQuantum` and reject their Promises on errors — see [Async Solving](#async-solving). |
| — (no v2 equivalent) | `SatSolver.add(expr)` conjoins constraints into a live handle, and `solver.variables()` returns the globally sorted named set — see [Adding Constraints Incrementally](#adding-constraints-incrementally). |
| — (no v2 equivalent) | Cardinality constructors `atMostOne`/`atMost`/`atLeast`/`exactly` — see [Cardinality Constraints](#cardinality-constraints). |

Two behavioral changes survive a mechanical port. First, restart timing is **history-dependent**: the deterministic EMA restart policy carries its fast/slow LBD histories across searches on one handle, so a later call's restart schedule can reflect earlier calls' learning (v2 restarted on a per-search Luby schedule). Second, retained learned clauses, saved phases, PLE, and index history all influence search, so a valid v3 first model need not equal v2's — preserve forced-value/validity checks rather than depending on incidental model choices.
...
- `npm run bench` is the **v3 candidate harness**. It authenticates the sealed references exclusively from pinned Git bytes — the v2 baseline `test/v3-baseline.json` and `test/legacy-compiled-cnf.json` at their artifact commit, plus the frozen recorder overlay at its seal commit — then regenerates the pinned 23-row corpus (random 3-SAT at ratio 4.26, PHP(9,8), XOR parity chains, seeded graph coloring, a 32-call incremental history, and `pairs(8)` enumeration), verifies immutable scenario identities and calibrated caps, and measures the candidate once per scenario. On success it **writes only `test/v3-benchmark.json` and `test/v3-benchmark.md`**. Gates mode (the default) enforces immutable-input/verdict/oracle/cap checks and discloses algorithmic deltas; `--parity` hard-fails on any behavioral mismatch and is retained for explicit parity experiments.
- `npm run bench:legacy` is the **assert-only** legacy verifier and **writes nothing**. It authenticates the frozen Phase-1/2/3 evidence plus the Phase-4 record, report, and manifest at their pinned commit — including every embedded Phase-4 source fingerprint against those Git bytes — re-runs all 8 legacy fixtures, independently checks SAT models, and compares the current compiler's complete canonical compiled snapshots against the authenticated pristine-v2 references. Gates mode (the permanent default) keeps the same authentication/verdict/oracle/cap checks and prints non-fatal counter deltas; `--parity` hard-fails unless all 48 recorded counters exactly match the frozen record. HEAD is context, not a substitute for source fingerprints.
- The eight legacy fixtures are the hypergraph, PHP(5,4)/(6,5)/(7,6)/(8,7), and 20-variable/85-clause random 3-SAT with seeds 42/43/44. Original six-row conflict caps stay 200,000; the larger PHP caps stay **7,230/36,270**, from fixed historical calibrations. Budget exhaustion surfaces as `'unknown'` and is a measurement failure, never UNSAT evidence. These modest fixed cases — and the v3 corpus alike — do not establish general scalability, incremental speedups, default reduction engagement, or total-memory bounds.
- Counter ratios are not wall-time speedups. The reports explicitly retain regressions as well as gains, and every counter/model/compiler-output delta between the sealed v2 baseline and the candidate is disclosed, never tuned away silently. See the [v3 benchmark review](test/v3-benchmark-review.md) for the per-task analysis and its limitations; the frozen `test/phase*-benchmark-review.md` files remain as historical v2 evidence. Timing is console-only, not a test gate.
